...
- **Владельца маршрута спрашиваем у ядра, а не у таблицы.** `wg-quick` кладёт маршрут
  по умолчанию не в главную таблицу, а в свою (51820) с правилом `ip rule`. Дамп
  `/proc/net/route` в такой раскладке показывает старый маршрут через Ethernet, и охрана
  завершила бы цели при полностью исправном VPN. Ответ даёт UDP-`connect` на публичный
  адрес плюс `getsockname`: ядро выполняет полный поиск маршрута с учётом правил,
  не отправив ни байта. Закреплено `policy-routing-contract.sh`.
- **VPN — по имени интерфейса, а квалификация по данным ядра** (`DEVTYPE`, `tun_flags`,
  ARPHRD), не по имени: «VPN» пользователь напишет на чём угодно. Незнакомое имя → `Down`.
- **`is_up` — это только бит IFF_UP.** IFF_RUNNING в sysfs не отражается, а у туннелей
  понятие «несёт линк» не определено. Идёт ли через интерфейс трафик, отвечает проба маршрута.
- **События сети — подписка на netlink**, замена `NWPathMonitor`. Реакция — единицы
  миллисекунд против пяти секунд у штатного тика; закреплено `netlink-events-contract.sh`.
- **Поллинг 250 мс остаётся:** ловить запуски через netlink-коннектор нельзя, он требует
  `CAP_NET_ADMIN`, а вся установка задумана беспривилегированной.
- **Ловушки macOS решаются ядром:** `readlink /proc/<pid>/exe` отдаёт уже разрешённый путь
  (симлинк `nano`→`pico` не возникает), argv лежит готовым массивом в `cmdline`.
