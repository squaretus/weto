# Архитектура weto

Один продукт в двух реализациях: macOS (Swift) и Linux (Rust). Кода они не делят —
только данные: токены дизайна, исходники иконки и голден-фикстуры политики.

## Стек macOS
- Swift 5.9 (tools-version 5.9), Swift Package Manager, target macOS 26.0+ (Apple Silicon).
- SwiftUI: `MenuBarExtra` со стилем `.window` плюс окно настроек в том же процессе.
  Отдельного GUI-таргета нет. VM-слой `@Observable @MainActor`, один `AppCoordinator`.
- Системные API: `SCDynamicStore` (сетевые сервисы), `NWPathMonitor` (смена пути),
  `libproc` + `KERN_PROCARGS2` (процессы и их командные строки), Keychain, `UserNotifications`.
- Внешних зависимостей нет. Флаги стран тянутся с CDN `HatScripts/circle-flags`.
- Распространение: PKG-установщик, автообновление через GitHub Releases `squaretus/weto`.

## Стек Linux
- Rust 2021 (floor 1.82), Cargo workspace в `linux/`, GTK4 без libadwaita.
- Системные API: sysfs (`/sys/class/net`), `/proc`, netlink (события сети),
  UDP-`connect` для поиска маршрута, D-Bus (трей, уведомления).
- Цели: Ubuntu 24.04 LTS и Arch, x86_64. Пол GTK4 — 4.14.
- Установка целиком в `$HOME`: `~/.local/share/weto/<версия>` плюс симлинк `current`.
  Обновление — распаковка рядом и атомарная подмена симлинка.
- Сборка и тесты идут в Linux-контейнере: `linux/scripts/dev.sh cargo test --workspace`.
  Кросс-компиляции недостаточно — тесты обязаны выполняться на настоящем ядре.

## Персистентность
Доменной БД нет, правило UUID-PK неприменимо. Хранилища macOS:
- `UserDefaults(suiteName: "com.weto.shared")` — настройки и журнал (кольцевой буфер на 10 записей).
- Keychain, сервис `com.weto.ipinfo`, аккаунт `token` — токен ipinfo. В `UserDefaults` он
  не попадает намеренно: plist читается любым процессом пользователя и уходит в бэкапы.
- `~/Library/Caches/com.weto.app/flags-circle/` — SVG флагов.

Хранилища Linux (XDG): `~/.config/weto/config.toml` — настройки,
`~/.local/state/weto/journal.json` — журнал, `~/.cache/weto/flags-circle/` — флаги.
Токен ipinfo — отдельным файлом `~/.config/weto/token` с правами `0600`, в конфиг
он не попадает по той же причине, что не попадает в plist на macOS.

## Сборка и запуск
```bash
cd macos && swift build                              # Debug-сборка macOS
cd macos && swift test                               # Тесты
swift test --package-path macos/Packages/UpdateKit   # Тесты пакета обновления
swift run --package-path macos WetoMenuBar           # Запуск без бандла (без Keychain и уведомлений)
macos/scripts/make-app.sh release                    # .app → macos/.build/app/Weto.app
macos/scripts/build.sh 0.1.0                         # PKG → macos/.build/release_build/Weto-0.1.0.pkg

linux/scripts/dev.sh cargo test --workspace          # тесты Linux (в контейнере)
linux/scripts/dev.sh bash scripts/tests/app-smoke-contract.sh
linux/scripts/build.sh 0.1.0                         # tar.zst → linux/target/release_build/
```

## Раскладка репозитория
Продукт существует в двух реализациях, и они не делят ни строчки кода — только данные.
```
macos/     приложение под macOS: SPM-пакет, скрипты сборки и упаковки, ресурсы бандла
linux/     приложение под Linux: Cargo workspace (Rust + GTK4)
shared/    то, что обязано совпадать у обеих реализаций: исходники иконки,
           токены дизайна, голден-фикстуры политики
docs/      канон дизайн-системы — общий, по платформам не делится
```
Голден-фикстуры — единственный механизм, которым расхождение двух реализаций
ловится машинно: один и тот же набор случаев прогоняют оба тест-раннера.

## Структура macOS-части
```
macos/Sources/
├── WetoCore/     [library] чистая логика, ноль I/O: политика, снимки, матчинг, разбор ответов
├── WetoHelper/   [executable] точка входа демона: конфигурация + listener, логика в UpdateKit
├── WetoSystem/   [library] адаптеры к macOS, каждый за протоколом
├── WetoShared/   [library] VM-слой: координатор, машина состояний охраны, хранилища
├── WetoDesign/   [library] токены и компоненты дизайн-системы, лейбл менюбара
└── WetoMenuBar/  [executable] точка входа, попап и окно настроек
macos/Packages/UpdateKit/   переносимый между проектами механизм обновления, свой Package.swift
├── UpdateKitCore/    [library] ноль I/O: конфигурация, версии, политика показа, фазы, тексты
├── UpdateKitXPC/     [library] граница демона: протокол, клиент; зависимостей нет
├── UpdateKit/        [library] UpdateController, планировщик, хранилище, фетчер релиза
├── UpdateKitUI/      [library] окно обновления и протокол темы
└── UpdateKitHelper/  [library] root-логика: перепроверка релиза, загрузка с долей, installer
macos/Tests/          WetoCoreTests, WetoSystemTests, WetoSharedTests, WetoDesignTests
macos/scripts/tests/  shell-контракты установки и релизной сборки (из build.sh, вручную и из CI)
```
Состав файлов каждого таргета — в его документе из индекса ниже.

## Структура Linux-части
```
linux/crates/
├── weto-core/    ноль I/O: политика, снимок сети, матчинг, CIDR, гео-модели, презентация
├── weto-sys/     адаптеры за трейтами: sysfs, /proc, netlink, HTTP, секрет, завершение,
│                 уведомления, автозапуск
├── weto-config/  настройки (TOML), журнал (JSON), пути XDG
├── weto-guard/   машина состояний охраны и применение решения к процессам
├── weto-ui/      дизайн-система на GTK4: сгенерированные стили и компоненты
├── weto-tray/    иконка StatusNotifierItem и её рендер из общего исходника
├── weto-update/  самообновление: проверка релизов, политика показа, установка, откат
├── weto-app/     [bin weto] точка входа, окно статуса, окно настроек
└── wetod/        [bin] испытательный стенд без UI, в продукт не поставляется
linux/scripts/    dev.sh (сборка в контейнере), build.sh, install.sh, tests/ — контракты
```
Инвариант границ `weto-core` тот же, что у `WetoCore`, и проверяется машинно:
`scripts/tests/core-boundary-contract.sh` валит сборку при появлении tokio, zbus,
reqwest, netlink, procfs, gtk4 или rustix в графе зависимостей ядра.

## Индекс модулей
- **WetoCore** ([docs](../docs/modules/weto-core.md)) — `GuardPolicy.decide`/`decideLocal`,
  статус VPN из снимка, отбор процессов с обходом дерева потомков, CIDR, разбор ответов
  гео-сервисов и GitHub Releases, отчёт о пробе (`GeoProbeReport`) и классификация отказа
  (`GeoFailure`). Не импортирует системные фреймворки — инвариант проекта.
- **UpdateKit** ([docs](../docs/modules/update-kit.md)) — весь механизм обновления одним
  переносимым пакетом из пяти таргетов: политика показа и фазы (`UpdateKitCore`), протокол
  демона `performUpdate`/`installState`/`uninstallHelper` (`UpdateKitXPC`), `UpdateController`
  (`UpdateKit`), окно и тема (`UpdateKitUI`), root-логика (`UpdateKitHelper`). Ни одной
  константы проекта внутри: всё приходит `UpdateFeedConfiguration`.
- **WetoHelper** ([docs](../docs/modules/weto-helper.md)) — LaunchDaemon `com.weto.helper`
  под root: `main.swift` на 15 строк собирает `UpdaterHelperService` из конфигурации weto.
  Демон сам опрашивает GitHub, качает `.pkg` в `/var/db/weto/updates` (0700, файл 0600),
  ставит через `installer -pkg`, отдаёт фазу и долю. Клиента авторизует по пути исполняемого
  файла: Developer ID нет, сверять team-id нечем. От root это не защищает — предел осознан.
- **WetoSystem** ([docs](../docs/modules/weto-system.md)) — адаптеры к macOS, каждый
  за протоколом: только на этой границе тесты и подменяют что-либо.
- **WetoShared** ([docs](../docs/modules/weto-shared.md)) — `GuardController` (машина
  состояний, ревизии, владение пробой), `ProcessEnforcer` (кэш правил, один обход, завершение),
  `GuardVM` как фасад для UI, хранилища и обслуживание.
- **WetoDesign** ([docs](../docs/modules/weto-design.md)) — токены и компоненты
  (`docs/design-system.md`), `MenuBarImageRenderer`. Компоненты не знают о состоянии приложения.
- **WetoMenuBar** ([docs](../docs/modules/weto-menubar.md)) — UI и точка входа: попап
  со статусом, гео, живыми целями, кнопкой проверки и баннером обновления; управление —
  в окне с двумя вкладками.

- **Linux-охрана** ([docs](../docs/modules/linux-guard.md)) — те же границы на Rust:
  `weto-core` с политикой и общими фикстурами, `weto-sys` с адаптерами к sysfs, `/proc`
  и netlink, `weto-guard` с машиной состояний. Ручные проверки — `linux/docs/manual-check.md`.

Потоки данных — [overview](../docs/overview.md). Процедуры — [runbooks](../docs/runbooks/).

## Ключевые контракты
- **Порядок проверок политики** (задаёт и приоритет причины, и экономию запросов): охрана
  выключена или целей нет → VPN не выбран → не поднят → не держит default route → ipinfo молчит
  → IP в чёрном списке → страна от ipinfo заблокирована → нет подтверждения → страна
  от подтверждающего заблокирована → страны расходятся → безопасно.
- **Fail-closed строгий:** отсутствие подтверждения страны завершает цели, даже когда ipinfo
  уверенно сообщил безопасную страну. Осознанное решение владельца.
- **Fail-closed до вердикта:** потерявший свежесть вердикт (холодный старт, смена сетевого пути,
  правка настроек) даёт `verificationPending` — цели завершаются ещё до запроса к ipinfo.
  Свежесть — пара «ревизия конфигурации + отпечаток снимка сети»: без неё штатный тик каждые
  5 секунд убивал бы цели при исправном VPN.
- **Устаревший результат не возвращает safe:** у каждой пробы своя ревизия, а настройки
  и снимок читаются непосредственно перед применением.
- **VPN — по UUID сервиса и квалификации SystemConfiguration**, не по имени: его переименовывают,
  а Wi-Fi с именем «VPN» иначе сошёл бы за туннель. Незнакомый UUID → `.down`.
- **Туннель без сервиса опознаётся по интерфейсу.** Клиент, поднимающий `utun` сам
  (сборки на Xray мимо App Store), сетевого сервиса не создаёт и в `Setup:/Network/Service/…`
  не появляется — выбрать его было нечем. Такие интерфейсы идут в кандидаты отдельно,
  с идентификатором `interface:<имя>`, и квалифицируются как на Linux: тип устройства
  плюс назначенный адрес IPv4. Адрес обязателен — служебные `utun` (частный узел iCloud,
  AirDrop) живут с одним link-local IPv6 и иначе засорили бы список.
- **Владельца маршрута спрашиваем и по сервису, и по интерфейсу** (`PrimaryService`
  и `PrimaryInterface`): у туннеля без сервиса первый назовёт подлежащий Wi-Fi, хотя
  маршрут уже ушёл в туннель, — и охрана завершала бы цели при исправном VPN.
- **Выбор VPN переживает исчезновение интерфейса.** Туннель, поднятый клиентом
  самостоятельно, существует ровно пока живо подключение: выключил VPN — интерфейса
  нет. Список, собранный из одних живых кандидатов, в этот момент терял выбор,
  а обработчик списка записывал потерю в настройки — и после возвращения VPN охрана
  считала, что сервис не выбран, завершая цели уже без причины. Выбранный туннель
  остаётся в списке отдельной строкой «(не подключён)», а перестройка списка
  и установка выбора отделены от нажатия пользователя (`vpn_rows` в `weto-core`,
  `VPNPicker` в `WetoCore`). Статус при этом — `Down`, то есть fail-closed.
- **Имена `utun` не закреплены за приложением** и меняются между запусками. Исчезнувший
  интерфейс — `.down`, то есть fail-closed: подставлять «наверное, это вон тот другой
  туннель» нельзя. Цену этого платит пользователь такой сборки — выбор приходится
  повторять; у сервиса из App Store UUID постоянен.
- **Локальное против сетевого:** сначала `decideLocal` — падение VPN видно из `SCDynamicStore`
  мгновенно. В сеть идём только при отсутствии локальных оснований, коалесценция 300 мс.
- **Экономия запросов относится к вердикту, а не к экрану.** Показания гео
  обновляются и тогда, когда судьба целей решена локально: один запрос на каждую
  смену пары «ревизия + отпечаток сети», и только пока охрана на посту (включена
  и цели заданы). Пока экономию распространяли и на показания, после падения VPN
  на экране навсегда оставались адрес и страна туннеля — то есть экран показывал
  защиту, которой уже нет. Устаревшие показания при этом гасятся до ответа сети:
  прочерк честнее чужой страны. Вердикт от такого запроса не смягчается никогда.
- **Проверка по кнопке не стоит пользователю целей:** `GuardController.probeNow()` уходит
  в сеть без окна коалесценции и не сбрасывает свежесть вердикта, поэтому нажатие при
  исправном VPN не роняет состояние в `verificationPending`. Ответ применяется обычным путём,
  так что восстановившийся сервис снимает блокировку сразу. Повторное нажатие в полёте
  запроса не порождает — у подтверждающего сервиса лимит.
- **Кнопка спрашивает «где я сейчас», а не «нужна ли охране проверка».** Запрос уходит
  и тогда, когда судьба целей решена локально: VPN выключен, сервис не выбран, охрана
  выключена или целей нет. Экономия запросов — свойство штатного тика; на кнопке она
  означала бы молчание попапа ровно в тот момент, когда пользователь и хочет увидеть
  свою страну. Локальное основание при этом применяется сразу, до ответа сети,
  поэтому целям кнопка жизни не продлевает.
- **Проба возвращает отчёт, а не голый вердикт** (`GeoProbeReport`): исход по каждому
  источнику, признак сети от `NWPathMonitor` и время. Вердикт охраны — `report.outcome`,
  поэтому показанное на экране и применённое к целям выведены из одного объекта.
  Классификация отказа (`GeoFailure`) живёт в `WetoCore`: с границы приходят только числа,
  а нераспознанный код показывается исходным текстом системы, а не выдаётся за «нет сети».
- **Гео-сервисы выбраны по измерениям, а не на веру**, и кэша на этом пути нет нигде:
  ipinfo как единственный источник IP для вердикта, подтверждение `free.freeipapi.com`
  с откатом на `get.geojs.io`. Лимиты и отвергнутые сервисы —
  [decisions/geo-confirmation-services](../docs/decisions/geo-confirmation-services.md).
- **Без токена ipinfo проба показывает справочную страну, но не выносит вердикт.**
  `get.geojs.io/v1/ip/country.json` отвечает про звонящего без токена и без адреса
  на входе, поэтому свежая установка сразу отвечает на вопрос «где я». В решение охраны
  этот ответ не идёт никогда: `GeoProbeReport.outcome` требует ответа ipinfo и остаётся
  `.unavailable`, то есть fail-closed.
- **Адрес из сети не попадает в URL как есть** — проверяется через `inet_pton`.
- **Скриптовая цель сверяется с argv поэлементно:** подстрочное сравнение убивало обёртки
  с похожим именем и процессы, у которых путь цели встретился в данных команды.
- **Правило цели живёт не дольше `targetRuleRefreshSeconds` (2 с).** Развёрнутый через
  симлинки путь бинарника обычно содержит номер версии, и обновление инструмента меняет его
  целиком: кэш правил по одному лишь списку целей означал, что цель молча выпадала и из
  списка запущенных, и из-под завершения — до повторного добавления руками. Обновлённая цель
  подтягивается сама; окно свежести оставлено, потому что разрешение стоит обращений
  к файловой системе и LaunchServices, а сторож обходит процессы каждые 250 мс.
- **Пересчёт правила не сужает охрану.** Правило помнит прежние пути запуска (сеанс,
  начатый до обновления, живёт на старом бинарнике и `proc_pidpath` сообщает про него старый
  путь), а цель, переставшая разрешаться в момент подмены файла, держится последней известной.
  Забытый путь на диске уже не существует, поэтому новый процесс по нему не появится.
- **Поллинг 250 мс при небезопасном состоянии** — единственный способ ловить терминальные
  процессы: `NSWorkspace` уведомляет только про GUI-приложения.
- **Журнал** пишет каждый новый pid и каждую новую причину в рамках эпизода: иначе запись
  «подключение ещё не проверено» съедала бы настоящую причину.
- **Старт охраны — в `AppDelegate`**, не в `.task` попапа: `MenuBarExtra` со стилем `.window`
  создаёт содержимое лениво.
- **Автозапуск — один файл** `~/Library/LaunchAgents/com.weto.app.plist`: его пишет
  `postinstall`, им же управляют тумблер в настройках и деинсталлятор. Системный
  `/Library/LaunchAgents` не пакуется и подчищается как наследие прежних версий.
- **Приложение не выгружает задание, которым само является.** Копия от launchd видит себя
  по `XPC_SERVICE_NAME=com.weto.app`; для неё `enable` только пишет plist, а `disable` только
  удаляет его. Иначе тумблер автозапуска означает SIGTERM самому себе. `bootout` остаётся
  у `Maintenance.closeApp`, который и намерен завершить приложение.
- **Резидентность объявлена явно** (`NSSupportsAutomaticTermination`/`NSSupportsSuddenTermination`
  плюс отказ в `AppDelegate`): иначе копию от launchd система усыпляет в первый момент без окон.
- **В Dock приложение приходит только с окном.** По умолчанию оно фоновое
  (`LSUIElement` плюс `.accessory`), но открытые настройки или окно обновления — обычные окна,
  и без иконки в Dock их не найти ни в Cmd+Tab, ни мышью. `DockPresence` слушает появление
  и закрытие окон и переключает политику активации; попап менюбара окном не считается
  (не титульный, не может стать главным) — иначе иконка мигала бы на каждое открытие меню.
- **Иконка приложения меняется вместе с темой** (`WetoAppIcon` + `AppCoordinator.applyAppIcon`):
  её видят Dock, `NSAlert` и окно обновления. Бандл несёт статичный `AppIcon.icns`; обе картинки
  и `.icns` собираются из `shared/icon/*.icon` скриптом `macos/scripts/build-icon.sh`.
- **Версия — из `Info.plist` бандла** (`Constants.appVersion` сверяет `CFBundleIdentifier`,
  иначе отдаёт `dev`). Релизная сборка не правит отслеживаемые файлы.
- **Ресурсы — в `Contents/Resources`, доступ через `DesignResources`:** `Bundle.module` смотрит
  лишь в корень бандла и в путь машины сборки, а ресурс в корне `.app` `codesign` пломбировать
  отказывается — подпись молча не создавалась.

### Контракты Linux-реализации

Политика, порядок проверок и оба правила fail-closed — общие с macOS и проверяются
одними фикстурами. Расходится только то, что диктует система, и три расхождения
стоят упоминания здесь; остальное — в [linux-guard](../docs/modules/linux-guard.md).

- **Владельца маршрута спрашиваем у ядра, а не у таблицы.** `wg-quick` кладёт маршрут
  по умолчанию в свою таблицу за правилом `ip rule`, и дамп `/proc/net/route` показал бы
  старый маршрут через Ethernet — охрана завершила бы цели при исправном VPN.
- **Root не нужен нигде:** ни установке, ни обновлению, ни завершению целей, ни чтению
  маршрутов. Привилегированного помощника в Linux-версии нет как класса. Защита загрузки
  при этом та же, что на macOS, — https и список хостов доставки GitHub; подписи нет
  ни там, ни здесь, и разводить платформы по её наличию было бы решением без причины.
- **Вид цели `appBundle` не переносится:** на Linux нет каталога, которым можно накрыть
  процессы разом; ярлык `.desktop` указывает на обычный бинарник, то есть `Binary`.
- **CSS-переменные не используются:** `var()` появился в GTK 4.16, а пол проекта — 4.14
  (Ubuntu 24.04 LTS). Значения подставляются генератором, по таблице стилей на тему.
- **Попап не привязан к иконке трея:** SNI не сообщает координат, Wayland не даёт окну
  позиционировать себя. Окно статуса открывается там, где решит композитор — единственное
  заметное отступление от macOS, и обойти его нечем.
- **UI — порт, а не своя версия.** Состав и порядок экранов повторяют macOS поэлементно:
  окно настроек — те же пять карточек («Цели», «Сеть и гео», «Чёрный список»,
  «Внешний вид», «Обслуживание») и тот же подвал; попап — щит, заголовок, две иконки,
  показания гео, баннер и живые цели. Отступления перечислены
  в [linux-guard](../docs/modules/linux-guard.md#the-ui-is-a-port-not-a-redesign)
  и исчерпываются четырьмя пунктами, каждый продиктован системой.
- **Тумблера охраны нет ни на одной платформе.** Поле `is_enabled` в модели настроек
  есть у обеих реализаций и наружу не выведено ни у одной; настройки «уведомлять
  о завершении» нет тоже — уведомление уходит всегда.
- **Штатная тема рисует кнопки градиентом.** Breeze и Adwaita кладут `background-image`
  поверх нашего `background-color`, и без явного `background-image: none` кнопка
  остаётся светлой, а светлый текст на ней исчезает. Сброс обязателен на каждом
  контроле, которому мы задаём заливку.

### Автообновление на Linux

Проверка на старте и раз в час, установка тем же процессом в домашний каталог.
Решение принимает та же чистая функция, что на macOS, и сверяется общими фикстурами.
Тихий исход прячет и окно, и баннер; автоустановка идёт молча; перезапуск — `exec`
поверх себя, пусковой симлинк к этому моменту уже указывает на новую версию.

- **Откат встроен:** отметка о попытке запуска ставится до создания окон и снимается
  через пять секунд жизни. Две неудачи подряд означают, что версия не стартует, —
  приложение возвращается к предыдущей само. На macOS такой страховки нет и не надо:
  там пакет проверяет установщик системы.
- **Ручная проверка игнорирует пропуск и отсрочку** — единственный и достаточный
  способ вернуть пропущенную версию, поэтому кнопки «снять пропуск» нет.

### Автообновление

Проверка — из приложения по HTTP (`UpdateController`), установка — только через демон:
`installer` требует root. Демон перепроверяет релиз сам, требует читаемую версию установленного
приложения и качает пакет лишь с доверенных хостов GitHub (`ReleasePackageURL`). Релиз без `.pkg`
до демона не доходит. Успех не отчитывается — установщик выгружает и приложение, и демон.
Подробно — [overview](../docs/overview.md#update-http-check-in-the-app-root-install-in-the-daemon).

- **Проверка раз в час и на старте**; интервал — поле `UpdateFeedConfiguration`, а не константа.
- **Что делать с находкой, решает чистая функция** `UpdatePolicy.decide`: `silent` (версия
  пропущена, отложена или не новее), `prompt` (окно и баннер) или `install` (молча, без окна).
  Тихий исход прячет и окно, и баннер.
- **Пропуск действует до версии выше пропущенной** и снимается сам — управлять им из настроек
  не нужно. Отсрочка хранится абсолютной датой и переживает перезапуск; дата дальше шести часов
  считается испорченной (перевод часов назад иначе запер бы обновления).
- **Ручная проверка игнорирует пропуск и отсрочку** — единственный и достаточный способ
  вернуть пропущенную версию.
- **Ход установки читается методом `installState`**: код фазы, доля скачанного, текст провала.
  Метод только читает — root-поверхность от него не растёт. Молчание демона не выдаётся
  за успех, а незнакомый код фазы читается как «идёт установка», а не как «простой».
  Доля честная только на загрузке: `installer` прогресса не отдаёт.
- **Автоустановка идёт молча** и опирается на `KeepAlive` агента: launchd поднимает
  приложение сразу после того, как установщик его завершил.

### Базовый образ

Отсутствует — нативное macOS-приложение на SPM с PKG-установщиком, Docker не используется.
