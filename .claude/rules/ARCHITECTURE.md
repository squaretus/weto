...
- **Каждая проверка подключения тоже оставляет след.** Журнал завершений отвечает
  ...
  пишется не всё: из расписания идёт только состоявшийся запрос без ответа, иначе
  полсотни записей стали бы четырьмя минутами истории. В интерфейс не попадает.
