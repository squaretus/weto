...
Last measurement on the owner's machine: profile `203.0.113.177` (KZ), RU-released
`203.0.113.231`. Write down your own pair before continuing — the rest of the procedure is a
...
```bash
route -n get 203.0.113.177          # or the candidate's resolved address
