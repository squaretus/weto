...
#
# Мерять надо на x86_64 — той архитектуре, под которую выходит релиз. На aarch64
# тот же код даёт заметно другое число, и база, снятая с машины разработчика
# на Apple Silicon, проверяла бы наугад.
#
# История правок базы — чтобы рост был осознанным:
#   8400  первая релизная сборка: GTK4, ureq с rustls, resvg, ksni (aarch64).
#  10311  самообновление (tar, zstd, filetime, xattr — распаковать релиз
#         без них нечем) и порт интерфейса под macOS: пять карточек настроек,
#         разрешение целей через ярлык и запускатор. Первое измерение на x86_64.
BINARY_BASELINE_KB=10311
