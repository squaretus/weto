...
pub fn run() -> Result<(), String> {
    ...
    let output = std::process::Command::new("bash")
        .arg(&script)
        // Себя деинсталлятору не отдаём. Он ищет работающие копии по имени
        // процесса и файлу под каталогом данных — а мы и есть такая копия,
        // и без пропуска SIGTERM летел бы нам самим. Мгновенной смерти нет:
        // обработчик уводит сигнал в штатный выход, — но ждёт он в очереди
        // главного цикла и разбирается сразу, как вернётся `output()`. При
        // неудачном удалении это стоило бы пользователю сообщения об ошибке:
        // оно выставлялось бы и тут же исчезало вместе с приложением, хотя
        // именно ради него приложение и остаётся открытым.
        .env("WETO_UNINSTALL_SKIP_PID", std::process::id().to_string())
        .output()
