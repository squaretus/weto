...
следующем входе в систему, а если выключен — сам он не вернётся и после перезагрузки. Цели,
стоящие на паузе, weto на выходе отпускает: замороженными они не остаются.
...
Это единственный выход, после которого проверить будет уже некому. На Linux то же самое
делает скрипт рядом с установленной версией:
