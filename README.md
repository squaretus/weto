...
где решит оконный менеджер: протокол трея не сообщает координат значка, и привязать окно
к нему нечем — единственное заметное отступление от macOS.
...
Рядом с обычными настройками ключ не лежит ни на одной из систем — файл настроек читает любой
процесс пользователя, и он уезжает в резервные копии:

- **macOS** — связка ключей (Keychain), сервис `com.weto.ipinfo`;
- **Linux** — отдельный файл `~/.config/weto/token` с правами `0600`. Связкой ключей рабочего
  стола (Secret Service) weto не пользуется.

Через несколько секунд окно статуса должно показать ваш адрес и страну.
...
![Выбор VPN-приложения](docs/images/install/05-choose-vpn.png)

В строке **«VPN-приложение»** карточки «Сеть и гео» можно указать ту программу, через которую
у вас идёт VPN (в примере это `Happ`). Нажмите **«Выбрать…»** и найдите её в папке
с приложениями — ровно так же, как добавляются цели.

Раньше здесь нужно было выбрать сам туннель — строку вида `utun6` или `wg0`. Ответить
на такой вопрос невозможно: имена ничего не значат, меняются при каждом переподключении,
а у части клиентов туннеля в системных настройках сети нет вовсе. Название приложения
вы знаете — на него и смотрим.
...
Ступени две, и они разные. Плохой результат проверки — это ещё не улика, и на него weto
отвечает паузой. Завершение наступает только по доказательству.

**Ступень первая — пауза.** Первый же ответ сервисов, из которого не следует, что выход тот же,
останавливает цели (`SIGSTOP`) вместе со всем их деревом процессов. Заголовок в окне статуса
меняется на **«Выход не подтверждён»**, рядом с каждой стоящей целью встаёт значок паузы
с отсчётом до конца минуты, и внизу написано, чего именно ждём. Поводов ровно три, и все три —
ответ сервисов, а не догадка:

- **«Не удалось определить внешний адрес: …»** — ipinfo не ответил, и адрес никем не назван;
- **«Адрес сменился на …, страна не проверена»** — ipinfo молчит, а запасной сервис называет
  другой адрес, чем прежде;
- **«Подтверждающие сервисы недоступны»** — адрес есть, а подтвердить страну нечем.

Пока цели стоят, проверки идут в прежнем ритме. Ответ «безопасно» снимает паузу (`SIGCONT`),
и цели продолжаются с того места, где стояли, — ничего не потеряно.

Цель, запущенная в терминале, может после этого вернуться в фон: её задание забрал себе шелл.
Тогда рядом со значком паузы появляется **(i)** с подсказкой «Процесс вернулся в фон. Откройте
терминал и введите `fg`», а если терминал есть чем поднять — ещё и кнопка **«Показать терминал»**.
На Linux кнопку получают не все эмуляторы: поднять окно умеет само приложение через сессионную
шину, и у тех, кто на шину не выходит (xterm, alacritty, kitty), остаётся подсказка.

**Ступень вторая — завершение.** Цели завершаются (`SIGKILL`, заголовок **«Небезопасно»**,
запуск запрещён до подтверждения безопасного выхода) в двух случаях: либо доказана утечка,
либо истекла минута паузы. Вопросы weto задаёт всегда в одном порядке, и он же задаёт
приоритет улики:

1. Охрана выключена или целей нет — безопасно, дальше не спрашиваем.
2. **«VPN-приложение не запущено»** — завершение; но только если вы его выбрали в шаге 8.
   Невыбранное VPN-приложение не является причиной ничего: охрана прекрасно работает
   по одному гео.
3. Вердикта нет или адрес не доказан — пауза («Не удалось определить внешний адрес…»
   или «Адрес сменился…»).
4. **«Адрес … в чёрном списке»** — завершение.
5. **«Обнаружена страна XX по данным ipinfo»** — страна из чёрного списка, завершение.
6. Страну нечем подтвердить — пауза («Подтверждающие сервисы недоступны»).
7. **«Обнаружена страна XX по данным …»** — то же, что пункт 5, но по данным подтверждающего
   сервиса, завершение.
8. **«Расхождение стран: ipinfo — …, подтверждение — …»** — сервисы называют разные страны,
   завершение.
9. **«Адрес … не входит в белый список»** или **«Страна XX не входит в белый список»** —
   если белый список непустой и выход не попал ни в разрешённые диапазоны, ни в разрешённые
   страны, завершение. Адресом объясняют тогда, когда диапазоны в списке есть; страной —
   когда список состоит из одних стран. На решение выбор формулировки не влияет.
10. Иначе — безопасно.

Особняком от этого списка стоит потолок паузы — **«Подтверждение не получено за 60 с»**.
Минута считается от плохого результата, продлить её нечем: «вердикта нет» результатом
не является, и смена сети под паузой отсчёт не перезапускает.

Ни холодный старт, ни смена пути в сеть целей не трогают: они обесценивают прежний вердикт
и просят новую проверку, а решает ответ на неё. Между сменой сети и ответом есть окно
в несколько секунд, когда цели работают без вердикта, — это принятая цена: прежнее «сперва
встали, потом спросили» останавливало цели каждую минуту при полностью исправном VPN.

Отдельный случай — «Помехи»: ipinfo молчит, но запасной сервис называет **тот же** адрес,
что и раньше. Это доказательство неизменности выхода, поэтому цели работают; заголовок
остаётся «На страже», а щит желтеет.

---

## Журналы

**Журнал завершений** — вкладка **«Журнал»** в окне настроек. Записей столько, сколько
процессов weto тронул: строка на процесс, а не на срабатывание. В каждой — цель и pid,
что с процессом сделали и почему, а третьей строкой диагностика: адрес, страны обоих
сервисов, чем процесс попал под охрану (правилом, как потомок такого-то, или как шелл,
вставший ради терминала цели) и итог.

Итог дописывается позже самой записи, и в этом весь смысл: эпизод, начавшийся паузой
и кончившийся ответом «безопасно», получает «возобновлено», а не остаётся навсегда
с отговоркой «сервисы не ответили». Выглядит запись так:

```
claude · pid 41207
на паузе — подтверждающие сервисы недоступны
IP: 203.0.113.28 · ipinfo: KZ · geojs: KZ · итог: возобновлено проверкой
```
...
**Журнал проверок** — второй файл, и в интерфейс он не попадает. Журнал завершений молчит
про «нажал проверить, и ничего не произошло»: проверка, никого не остановившая, записи там
не заводит. Второй журнал пишет как раз попытки — повод (кнопка, смена пути в сеть, правка
настроек, расписание, восстановление после падения), исход, длительность и ответ каждого
сервиса. Пятьдесят последних записей лежат в файле:
...
**Закрыть приложение** (карточка «Обслуживание») — weto завершится и перестанет охранять цели,
пока вы не запустите его снова; при выключенном автозапуске сам он не вернётся и после
перезагрузки. Настройки, журнал и автозапуск сохранятся, а цели, стоящие на паузе, weto
на выходе отпускает — замороженными они не остаются.

**Удалить приложение…** — полное удаление: приложение, автозапуск, настройки, журнал
и ключ ipinfo. Действие необратимо. Стоящие на паузе цели weto отпускает
раньше, чем сносит себя: вместе с учётом исчез бы последний, кто помнит, кому должен
`SIGCONT`. На Linux то же самое делает скрипт рядом с установленной версией:
