...
### Шаг 6. Выбрать VPN-приложение

![Выбор VPN-приложения](docs/images/install/05-choose-vpn.png)

В строке **«VPN-приложение»** укажите ту программу, через которую у вас идёт VPN
(в примере это `Happ`). Нажмите **«Выбрать…»** и найдите её в папке с приложениями —
ровно так же, как добавляются цели.
...
Пока это приложение не запущено, цели не работают: закрыли клиент — цели закрылись
в ту же секунду. Но запущенное приложение само по себе ещё не доказательство, что трафик
идёт через VPN, поэтому weto всё равно проверяет, из какой страны виден ваш адрес.
...
- VPN-приложение не выбрано в настройках;
- VPN-приложение не запущено;
- путь в сеть изменился и подключение ещё не проверено заново;
- ipinfo не отвечает, и ваш адрес при этом сменился;
