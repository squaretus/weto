...
private let geojsSelfKZ = Data(
    #"{"country":"KZ","country_3":"KAZ","ip":"203.0.113.56","name":"Kazakhstan"}"#.utf8
)
...
final class GeoProbeTests: XCTestCase {
    ...
    func test_missing_token_keeps_verdict_unavailable_but_asks_the_reference_source() async {
        ...
        XCTAssertEqual(report.ip, "203.0.113.56")
    ...
    func test_when_ipinfo_refuses_the_probe_asks_the_reference_source_for_its_own_address() async {
        ...
        XCTAssertEqual(report.ip, "203.0.113.56", "без адреса нечем доказать, что он не менялся")
        XCTAssertEqual(report.confirmation, .answered("KZ"))
        XCTAssertEqual(report.confirmSource, .geojs)
    ...
    func test_third_self_ip_source_names_the_address_when_the_first_two_are_silent() async {
        let fetcher = FakeFetcher(responses: [
            ...
            "geojs.io": .failure(FetchFailure()),
            "checkip.amazonaws.com": .success(Data("203.0.113.177\n".utf8)),
        ])
        let probe = GeoProbe(fetcher: fetcher, networkPath: FakeNetworkPath(hasPath: true), token: { "t" })

        let report = await probe.probe()

        XCTAssertEqual(report.ip, "203.0.113.177")
