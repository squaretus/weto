...
public enum Constants {
    ...
    /// ни одной попытки обновиться, дольше жёсткого (15 мин) не дало бы сервису
    /// вернуться раньше, чем годный ответ про этот адрес истечёт.
    public static let confirmationCooldownSeconds: TimeInterval = 300
    ...
    ///
    /// Заменять хост можно только на проверенный руками:
    /// `.claude/docs/runbooks/verify-geo-services.md`. Запасные, уже измеренные, —
    /// `api.country.is` и `ipwho.is`.
    ...
    /// Потолок паузы: дольше этого ждать подтверждения нельзя — пауза кончается
    /// либо возвратом (safe), либо завершением с уликой «подтверждение не получено».
    /// Считается от начала паузы, то есть от плохого результата пробы: стоящей фазы
    /// до результата у охраны нет.
    public static let pauseCeilingSeconds: TimeInterval = 60

    /// Сколько раз досылать SIGCONT записи, ответившей стопом на свой же сигнал.
    /// Настоящее фоновое задание отвечает так всегда: просыпается, читает tty
    /// и встаёт по `SIGTTIN`. Свои уведомление и запись weto не повторяет, а вот
    /// `notify` у zsh включён по умолчанию, и терминал печатал пользователю
    /// `suspended (tty input)` раз в секунду до самого `fg`. Обязательство при этом
    /// не снимается: запись остаётся в учёте, её исполняют завершение и штатный выход.
    public static let resumeRetryLimit = 3
