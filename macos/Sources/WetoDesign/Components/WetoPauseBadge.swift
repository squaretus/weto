...
public struct WetoPauseBadge: View {
    ...
    public init(deadline: Date?, hint: String?, onShowTerminal: (() -> Void)?) {
    ...
    public var body: some View {
        HStack(spacing: WetoTokens.space2) {
            // Секундный таймер живёт только внутри значка: остальная пилюля не перерисовывается.
            TimelineView(.periodic(from: .now, by: 1)) { context in
                Label {
                    Text(verbatim: Self.countdown(until: deadline, at: context.date))
                } icon: {
                ...
                .accessibilityLabel("на паузе, \(Self.countdown(until: deadline, at: context.date))")
            }
